...
| S51 | `startupCachePopulationMatrix.spec.ts:78`      | startup matrix keeps cold and warm small/large populations separate                                                   | Reclassified; cache/telemetry measurement       |
...
| S81 | `connectedWriterSwitching.spec.ts:580`         | Use this device switches A to B to A in place while preserving reader routes and the originating draft                | Strengthened; Reader Phase 3 verified           |
| S82 | `connectedWriterSwitching.spec.ts:745`         | an accepted server generation keeps its job and durable reply when Use this device transfers the writer               | Strengthened; Reader Phase 3 verified           |
| S83 | `connectedWriterSwitching.spec.ts:955`         | an empty server without Web Locks initializes only after the explicit setup action with a fresh writer identity       | Strengthened; Reader Phase 3 verified           |
...
| `connectedWriterSwitching.spec.ts`         | `activeWriterHeaders`: 422; `getAppliedServerResourceRevision`: 510; `getDatabaseSnapshot`: 767; `getStartupCoordinatorSnapshot`: 314, 334, 395, 1051, 1067; `waitForStartupMilestone`: 359, 1062                                                                                                                                                                                                                                                                        |
