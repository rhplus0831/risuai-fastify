...
| `server/fastify/src/maintenanceCoordinator.ts`, `backupCopyPool.ts`, `backupCopyWorker.ts` | Bounded save/staging/GC admission, native backup-copy workers, cancellation, and shutdown draining. |
...
| `server/fastify/src/remoteDiagnostics.ts`, `server/fastify/src/supportDiagnosticsAuth.ts` | Dedicated support authority, bounded sanitized reads, immutable cursor pages, and operator-only credential lifecycle. See [Diagnostics](diagnostics.md). |
...
or blocking startup recovery. The journal maintains retention incrementally,
uses operation-specific deadlines, restarts transient worker failures on a
bounded schedule, and records uncertain in-flight loss rather than replaying it.
App/history scope carries request and opaque
...
are owned by [Diagnostics](diagnostics.md#manual-client-diagnostics).
...
| Client diagnostics | `server/fastify/src/clientDiagnostics.ts`, `server/fastify/src/routes/browserDiagnostics.ts` | Authenticated, read-only recent-event snapshot and separately enabled bounded browser upload; no active-writer requirement. See [Diagnostics](diagnostics.md). |
...
| Assets/saves/backups | `assets.ts`, `save.ts`, `localFileImport.ts`, `realmImport.ts`, `backups.ts`, `storageUsage.ts` | Content-addressed assets, local character/module import, `.risu`/bundle/local-backup import/export, Realm import, snapshots, and authenticated read-only storage usage. Detailed persistence contracts live in [Assets And Saves](assets-and-saves.md). |
...
command transaction; the browser recovery path is documented in
...
If Stop arrives during prompt assembly before provider dispatch, the accepted
operation instead emits `done { outcome: 'cancelled' }`, persists no assistant
row, clears its durable `stopping` state, and releases the same-chat claim so a
follow-up send can proceed without a server restart. This is distinct from the
server cancel-before-acceptance tombstone durably requested by the browser; see
[Generation Client](../../src/docs/generation-client.md#operations-streams-and-reattach).
