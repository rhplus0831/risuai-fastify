...
Targeted source checks: 2026-09-15 (occupied-chat generation authority and integrated recovery proof).
...
| Occupied-chat generation and effects                               | Client `src/ts/server/generationOperations.test.ts` and the inlay/IGP/effect suites; server `chatOccupancy.test.ts`, `chatOccupancyEnforcement.test.ts`, `generationOperations.test.ts`, `generationMemoryOccupancyRecovery.test.ts`; browser `chatOccupancyInteraction.spec.ts`, `chatOccupancyRecovery.spec.ts`, `chatOccupancyCancellation.spec.ts`, `chatOccupancyInlays.spec.ts`, and `chatOccupancyIgp.spec.ts`                                                                                                                                                                                                                                                                              | Covers exact occupancy admission for owner and chat-only Send/Reroll/Stop, unsupported chat-only Continue/Regenerate, same-base revision retry/exhaustion, lost response and restart recovery, scoped Stop, finalization/effect/memory pins, atomic IGP, per-image inlay settlement, and rollout drain without general-write authority.                                                                                                                                                                                                                   |
...
The [chat occupancy browser suites](browser-state-sync-and-recovery.md#chat-occupancy-and-scoped-recovery)
extend this boundary to real separate BrowserContexts. They verify overlapping provider work
in different occupied chats, one-winner same-chat exclusion, retained occupancy
through observation/navigation, explicit switching, owner role transfer, and
exact transcript/operation/effect identity across response loss, suspension,
retry exhaustion, and restart. Rendered desktop/mobile-emulation controls prove
chat-only Send, Reroll, and Stop while Continue and general Regenerate remain
disabled. Deterministic local providers and controlled fault barriers make
duplicates and persistence observable; this is not evidence for live external
providers or physical-device eviction.

The sequential image-provider regression advances Chromium's clock beyond the
retired shared deadline while the second HTTP response is held. It verifies
that the request and preparation remain active, then releases the real response
and checks both persisted assets. This tests timer behavior without a fixed
31-second wall-clock sleep.
...
- The chat occupancy browser suites, occupied-operation client tests, and
  server occupancy/enforcement/memory-recovery suites jointly protect scoped
  multi-session admission and drain; ordinary same-owner concurrency is not
  multi-device authorization proof.
...
| Browser and explicit gates      | `server/fastify/browser-smoke/acceptedSendProtocol.spec.ts`; `server/fastify/browser-smoke/chatOccupancyInteraction.spec.ts`; `server/fastify/browser-smoke/connectedReaderGeneration.spec.ts`; `server/fastify/browser-smoke/debugEchoLayoutStability.spec.ts`; `server/fastify/browser-smoke/rerollSwipePersistence.spec.ts`; generation-relevant portions of `fastifyBrowserSmoke.spec.ts`; `src/ts/__tests__/renderCostHarness.test.ts`; `src/ts/__tests__/sendCloneCountProbe.test.ts`.                                                                                                                                                                                                                                |
