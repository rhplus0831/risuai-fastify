...
## Chat command coverage ownership

The original `src/ts/chatCommands.test.ts` coverage is now **232 cases across eight files**.
The split preserves all 231 original scenarios and adds an independent `sdData` dispatch regression.
Metadata expectations no longer derive their fixtures or expected patches from the production allowlist.
Duplicate-edit races await both command outcomes, and the sequence rejection case now fails after
an accepted first step and checks that the third step is skipped.

| File | Cases | Coverage owner |
| --- | ---: | --- |
| [chatCommands.test.ts](../../src/ts/chatCommands.test.ts) | 5 | Sequence wrappers, revision advancement, rejection, accepted-prefix reconciliation, and destructive-refresh fencing. |
| [chatCommands.metadata.dom.test.ts](../../src/ts/chatCommands.metadata.dom.test.ts) | 44 | Chat/folder metadata, translation and pin settings, independent patch contracts, field-scoped rollback, active-chat replacement, and retained metadata replay. |
| [chatCommands.organization.dom.test.ts](../../src/ts/chatCommands.organization.dom.test.ts) | 71 | Chat/folder creation, deletion, reset, forks, selection, ordering, stable row identity, note dependencies, and retained structural replay. |
| [chatCommands.generationSettings.dom.test.ts](../../src/ts/chatCommands.generationSettings.dom.test.ts) | 20 | Generation-settings saves, queued edits, durable predecessors, dependency ordering, sparse acknowledgements, and stale projection/rollback guards. |
| [chatCommands.messages.dom.test.ts](../../src/ts/chatCommands.messages.dom.test.ts) | 57 | Send/append, active-target freshness, chat snapshots, narrow message commands, incomplete transcript guards, concurrent rollback, bookmarks, and retained transcript replay. |
| [chatCommands.notesAndScriptstate.dom.test.ts](../../src/ts/chatCommands.notesAndScriptstate.dom.test.ts) | 19 | Scriptstate snapshots/variables, author notes, lifecycle keepalive, note-before-delete ordering, and retained note/scriptstate projections. |
| [chatCommands.imports.dom.test.ts](../../src/ts/chatCommands.imports.dom.test.ts) | 6 | Single/batch import staging, queue reservations, more than 100 chats, payload chunking, accepted prefixes, and oversized-message rejection. |
| [chatCommands.durableBatch.dom.test.ts](../../src/ts/chatCommands.durableBatch.dom.test.ts) | 10 | Batch persistence/settlement and shared chat/folder/message projection fencing across writer demotion and repromotion. |

[chatCommands.testSupport.ts](../../src/ts/chatCommands.testSupport.ts) shares mocks, setup/teardown,
response/deferred builders, and common transport/outbox fixtures; it registers no test cases.
Each suite imports it first and calls `setupChatCommandTests()`. This preserves mock installation and
the stores-before-database initialization order. Domain-specific fixtures stay with their suites.
The new `.dom.test.ts` suffixes preserve the original DOM runtime without extending the legacy routing list.
Run the exact owning file with `pnpm test -- <file>`; the original file now runs only sequence cases.

HTTP responses remain mocked here. The client transport, Fastify persistence, outbox/replay,
hydration, and UI suites retain their separate contracts. Domain-specific durable cases stay with
the corresponding domain; the batch suite owns only shared batch and cross-domain projection behavior.
...
- The [chat command suites](#chat-command-coverage-ownership) and `characterCommands.test.ts` are the main protection against broad rollback
...
| Chats and generation | `src/ts/activeChatGenerationSettings.test.ts`; `alternateGreetingCommands.test.ts`; `alternateGreetingMutation.test.ts`; `chatCommands.test.ts`; `chatCommands.metadata.dom.test.ts`; `chatCommands.organization.dom.test.ts`; `chatCommands.generationSettings.dom.test.ts`; `chatCommands.messages.dom.test.ts`; `chatCommands.notesAndScriptstate.dom.test.ts`; `chatCommands.imports.dom.test.ts`; `chatCommands.durableBatch.dom.test.ts`; `chatGenerationSettings.test.ts`; `globalApi.changeChatTo.test.ts`; `process/files/multisend.test.ts`; `src/ts/util.persona.test.ts` |
