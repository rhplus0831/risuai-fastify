...
| `Chat.parserDependencies.test.ts`, `Chat.editing.dom.test.ts`, `Chat.deletion.dom.test.ts`, `Chat.partialEditing.dom.test.ts`, `Chat.generationFeedback.dom.test.ts`, `BackgroundDom.parserDependencies.test.ts`, `ChatBody.parseMemo.test.ts`, `ChatBodyParseMemo.dom.test.ts`, and `ChatBody.svelte.test.ts`. | Avoids unrelated reparsing while invalidating on changed rows, greetings, chat/scriptstate/prompt regex/module/reload inputs; distinguishes no-op/changed inline edits; stable-ID deletion; stale partial edits; reuses parse/LLM work; handles translation/parser failure without retry amplification. | High correctness and performance value. |
