...
| Encrypted outbox persistence and recovery | `src/ts/server/pendingMutationOutbox.test.ts` covers encrypted persistence, unreadable-row counting, ordering, replacement/dispatch races, predecessor closure, receipts, ownership quarantine, and lineage changes. | Protects unsent edits and their exact replay identity across storage failures and ownership changes. |
