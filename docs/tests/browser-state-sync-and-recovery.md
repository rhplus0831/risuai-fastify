...
Targeted source check: 2026-09-20 (focused outbox test ownership).
...
`src/ts/server/readerReadAuth.svelte-node.test.ts`, `src/ts/readerProjectionLifecycle.test.ts`,
...
`visibleStateRecovery.spec.ts` keeps separate role-first forced-reload and
...
| --- | --- | --- |
| Writer identity and runtime bootstrap | `src/ts/server/activeWriterSession.test.ts`: generated session reuse after reload, invalid stored IDs, unavailable `sessionStorage`, and one-time notification/reload after terminal predecessor rollback loss. `src/ts/server/bootstrap.svelte-node.test.ts`: authenticated writer bootstrap, read-only bootstrap, malformed job filtering, HTTP/network errors, and initialized/non-negative revision validation. | Protects the first ownership and revision decisions made by every tab. Incorrect behavior can orphan local work or let a stale writer mutate state. |
| Startup capabilities and reader lifecycle | `src/ts/startupReadiness.test.ts`, `readerRouteIntent.test.ts`, `readerProjectionLifecycle.test.ts`, `src/lib/Workspace.svelte.test.ts`, `src/ts/bootstrap.readiness.dom.test.ts`, `src/ts/bootstrap.writerLifecycle.dom.test.ts`, `src/ts/bootstrap.writerSwitching.dom.test.ts`, and App route DOM coverage: monotonic milestones, narrow capability gates, denial/promotion, writer loss, local-only route intent, auth/replacement clearing, and accessible status/focus. | Allows managed Reader navigation while gating writer route effects, mutation and generation; failed promotion cannot replay completed writer work. |
| Encrypted outbox persistence and recovery | `src/ts/server/pendingMutationOutbox.test.ts` covers binary-aware plaintext exclusion, authenticated metadata tampering, and nonce freshness for ordinary staging and exact replacement in both WebCrypto and raw-key modes, plus unreadable-row counting, ordering, replacement/dispatch races, predecessor closure, receipts, ownership quarantine, and lineage changes. | Protects unsent edits and their exact replay identity across storage failures and ownership changes. |
| Outbox intent and durable route policy | `src/ts/server/pendingMutationOutbox.intent.test.ts` covers owned JSON snapshots, request/payload bounds, dependency metadata, accepted routes, rejected near-misses, and confirmed BardWiki rebuilds. | Keeps retained requests bounded and eligible for durable replay. |
| Outbox occupied-chat authority | `src/ts/server/pendingMutationOutbox.occupancy.test.ts` covers lane isolation, scoped generation admission, authority normalization, expired epochs, and Stop settlement after rollout changes. | Prevents occupied-chat authority from admitting general-owner writes or replaying stale generation controls. |
| Outbox projection ownership | `src/ts/server/pendingMutationOutbox.projection.test.ts` covers concrete targets, field-independent fences, accepted baselines, history compaction, replacement ordinals, and retirement. | Prevents older acknowledgements from overwriting newer local edits. |
| Outbox staging work | `src/ts/server/pendingMutationOutbox.workCosts.svelte-node.test.ts` measures normalization and envelope serialization for staging and prepared-intent replacement while asserting caller edits cannot change the retained snapshot. | Guards bounded browser work without losing snapshot ownership. |
...
## Chat hydration coverage ownership

The original `src/ts/server/chatMessageHydration.test.ts` coverage is now **125 cases across six files**.
All original 105 cases remain represented. Eighteen new cases isolate wrong-chat responses, exact
operation/result identities at each completion barrier, reroll-only changes, concurrent request
sharing and retry, and lorebook reset races. Separating bulk retry and strict-failure scenarios adds
two registered cases; lorebook batch tests also verify that only one request runs at a time.

| File (under `src/ts/server/`) | Cases | Owned behavior |
| --- | ---: | --- |
| [chatMessageHydration.test.ts](../../src/ts/server/chatMessageHydration.test.ts) | 39 | Active and explicit targets, full/tail/window reads, placeholders, owner state, cache/force/invalidation, concurrent reads, strict failures, and loading/error diagnostics. |
| [chatMessageHydration.reader.dom.test.ts](../../src/ts/server/chatMessageHydration.reader.dom.test.ts) | 21 | Certified reader bodies, writer overlays, promotion/reroll handoff, metadata clones, exact generation suffixes, cancellation, session changes, and deleted chat incarnations. |
| [chatMessageHydration.freshness.dom.test.ts](../../src/ts/server/chatMessageHydration.freshness.dom.test.ts) | 13 | Revision floors, overlapping/disjoint ranges, retained edits, local acknowledgements, translations, optimistic rollback, and reroll-only freshness. |
| [chatMessageHydration.bulk.dom.test.ts](../../src/ts/server/chatMessageHydration.bulk.dom.test.ts) | 14 | Sequential chat batches, request budgets, partial windows, Hypa data, background rerolls, reset/stale responses, missing entries, retry, and strict failures. |
| [chatMessageHydration.completion.dom.test.ts](../../src/ts/server/chatMessageHydration.completion.dom.test.ts) | 19 | Accepted-send authoritative completion, exact operation/result identity in fetched and newer resident replies, retained-projection post-apply verification, revision/range/target checks, and aborts. |
| [characterLorebookHydration.dom.test.ts](../../src/ts/server/characterLorebookHydration.dom.test.ts) | 19 | Character lorebook loading, stub transitions, concurrent reads, local-edit/reset fences, sequential batches, request budgets, missing entries, retry, and strict failures. |

`chatMessageHydration.testFixtures.ts` shares data builders only; mocks and lifecycle hooks remain in
each suite. The extracted `.dom.test.ts` files retain the original DOM runtime without extending the
legacy routing list. `chatMessageHydration.reactivity.svelte.test.ts` separately owns reactive consumers;
read-adapter tests own wire validation, and browser journeys own reload and visible reroll behavior.

## Browser command coverage ownership

The original `src/ts/server/commands.test.ts` coverage is now **167 cases across 21 files**.
All original 161 scenarios remain represented. Splitting the six malformed compact-settings variants
into named cases adds five registered cases, and an exact value-free receipt adds one positive control.
The strengthened assertions check ordinary writer identity, the receipt ACK destination, and complete
prompt-item projection epochs and owner state. Each malformed-settings case enables acknowledgement
and supplies a valid epoch so it reaches receipt validation.

| File (under `src/ts/server/`) | Cases | Owned behavior |
| --- | ---: | --- |
| [commands.test.ts](../../src/ts/server/commands.test.ts) | 21 | Authentication and writer identity, readiness, initialization/onboarding, malformed success receipts, revision caching/conflicts, and HTTP rejection classification. |
| [commands.queue.test.ts](../../src/ts/server/commands.queue.test.ts) | 20 | Queue ordering, coalesced reconciliation, atomic sequences, retained wrappers, accepted-prefix cleanup, and factory/refresh rollback. |
| [commands.durableReplay.test.ts](../../src/ts/server/commands.durableReplay.test.ts) | 7 | Stable receipt identities, inline predecessors, retry revisions, lineage recovery, and the ACK route/body/headers. |
| [commands.settings.test.ts](../../src/ts/server/commands.settings.test.ts) | 22 | Settings grouping, scalar and shallow-object payloads, compact acknowledgements, partial acceptance, and rollback. |
| [commands.presets.test.ts](../../src/ts/server/commands.presets.test.ts) | 10 | Legacy and split-preset dispatch, selection/order, sparse PATCH effects, and invalid acknowledgements. |
| [commands.modelProfiles.test.ts](../../src/ts/server/commands.modelProfiles.test.ts) | 2 | Model profiles, role/runtime defaults, provider credentials, and legacy conversion wire contracts. |
| [commands.agentPresets.test.ts](../../src/ts/server/commands.agentPresets.test.ts) | 17 | Standalone Agents, preset uses/steps/defaults, exact metadata/step effects, and malformed receipts. |
| [commands.prompts.test.ts](../../src/ts/server/commands.prompts.test.ts) | 6 | Prompt settings ownership and acknowledgements; prompt item payloads and complete owner-state/epoch effects. |
| [commands.personas.test.ts](../../src/ts/server/commands.personas.test.ts) | 5 | Persona dispatch, structural/digest certificates, PATCH effects, and cascade fallback. |
| [commands.translatorPresets.test.ts](../../src/ts/server/commands.translatorPresets.test.ts) | 6 | Translator preset dispatch, sparse effects, legacy mirrors, malformed receipts, and persona/translator lifecycle keepalive. |
| [commands.loadouts.test.ts](../../src/ts/server/commands.loadouts.test.ts) | 3 | Loadout dispatch, opt-in effects, and strict create/delete acknowledgements. |
| [commands.characters.test.ts](../../src/ts/server/commands.characters.test.ts) | 7 | Character creation sanitization, collection/selection/order effects, PATCH payloads, and refresh fences. |
| [commands.chats.test.ts](../../src/ts/server/commands.chats.test.ts) | 6 | Chat/folder dispatch, all-chat reset, structural acknowledgements, scriptstate, and selection/metadata effects. |
| [commands.translation.test.ts](../../src/ts/server/commands.translation.test.ts) | 7 | Message/greeting translation, immediate reconciliation, own-event deferral, and direct response/failure scopes. |
| [commands.messages.test.ts](../../src/ts/server/commands.messages.test.ts) | 1 | Message append/edit/delete/truncate/tail/replace and generation-result payloads, preconditions, and local effects. |
| [commands.chatGenerationSettings.test.ts](../../src/ts/server/commands.chatGenerationSettings.test.ts) | 4 | Full/sparse generation settings payloads, independent SHA-256 checks, and exact compact receipt reconstruction. |
| [commands.lorebooks.test.ts](../../src/ts/server/commands.lorebooks.test.ts) | 7 | Global and scoped lorebook routes, sparse entry certificates, structural effects, and authoritative fallback. |
| [commands.definitions.test.ts](../../src/ts/server/commands.definitions.test.ts) | 5 | Script/trigger replacement and compact mutations, client-only final arrays, revision matching, and opt-in effects. |
| [commands.modules.test.ts](../../src/ts/server/commands.modules.test.ts) | 4 | Module records/folders/order/enablement, compact effects, and missing-epoch or mismatched-response fallback. |
| [commands.plugins.test.ts](../../src/ts/server/commands.plugins.test.ts) | 4 | Plugin records/provider/order, cross-resource deletion fallback, and plugin-storage payloads/effects. |
| [commands.bardWiki.test.ts](../../src/ts/server/commands.bardWiki.test.ts) | 3 | BardWiki document/settings routes and fences, exact eventless previews/dry runs, and malformed receipts. |

`src/ts/server/commands.testSupport.ts` shares transport mocks, fetch capture, deferred/JSON/digest
helpers, and startup/revision cleanup; it registers no cases. Each suite imports it before command
runtime imports. Domain fixtures and expected responses stay beside their assertions. The replay
suite mocks the ownership-reload notification boundary and asserts it is requested on a lineage
mismatch; writer-session suites own the subsequent UI/reload lifecycle.

These files retain the frontend Node runtime and do not change core-tag selection. The command
adapters stop at HTTP requests and reconciliation payloads: domain-owner suites own state/rollback,
bootstrap suites own applying effects or falling back to reads, and Fastify suites own persistence.

## Bootstrap coverage ownership
...
| --- | ---: | --- |
| `src/ts/bootstrap.test.ts` | 22 | Initial startup, locale, initialization races, outbox/receipt/replay order, and writer readiness. |
| `src/ts/bootstrap.readiness.dom.test.ts` | 20 | Plugin/generation/chat readiness, retry/discard, stale target completion, push/display effects, and deferred cleanup. |
| `src/ts/bootstrap.writerLifecycle.dom.test.ts` | 29 | Automatic acquisition, demotion, auth loss, foreground recovery, replacement, and cancelled work. |
| `src/ts/bootstrap.writerSwitching.dom.test.ts` | 36 | Explicit setup/promotion, confirmation, supersession, retained work, route preservation, and teardown. |
| `src/ts/bootstrap.resourceEvents.dom.test.ts` | 31 | Event ordering/deduplication, replacement/gaps, cursors, memory/occupancy wiring, writer lineage, heartbeat, and reconnect. |
| `src/ts/bootstrap.acknowledgements.settings.dom.test.ts` | 22 | Chat generation settings, settings groups, and Agent Preset acknowledgements and fallback. |
| `src/ts/bootstrap.acknowledgements.presets.dom.test.ts` | 35 | Legacy/model/prompt/translator presets and prompt-owner acknowledgements and fallback. |
| `src/ts/bootstrap.acknowledgements.collections.dom.test.ts` | 24 | Persona, lorebook, and loadout acknowledgements and fallback. |
| `src/ts/bootstrap.acknowledgements.extensions.dom.test.ts` | 9 | Plugin storage/provider and module collection/definition acknowledgements and fallback. |
| `src/ts/bootstrap.acknowledgements.charactersAndChats.dom.test.ts` | 18 | Character definitions/collections, chat structure/selection, message mutation/translation, and stale-projection fallback. |
| `src/ts/bootstrap.errors.dom.test.ts` | 3 | Global error and rejection filtering. |

The four existing `core` cases remain tagged: three in `src/ts/bootstrap.test.ts` and
one in `src/ts/bootstrap.resourceEvents.dom.test.ts`. The file-level core inventory includes
both files. New suites use the explicit DOM suffix; the original startup file retains its
existing DOM routing.
...
- `commands.test.ts` classifies HTTP 400 (`invalid-request`), 404 (`not-found`), 401, 403,
  429, and 500 without treating transient failures as terminal. `commands.agentPresets.test.ts` metadata
...
- The [bootstrap suites](#bootstrap-coverage-ownership) expand raw pending-row counts `1`/`null`; prompt acknowledgement event failures
...
The accepted route cases in `pendingMutationOutbox.intent.test.ts` are grouped by behavior below. Each method/path pair is an individual
parameterized case:
...
- `src/ts/bootstrap.resourceEvents.dom.test.ts`, the bootstrap acknowledgement suites, the
  [browser command suites](#browser-command-coverage-ownership), and
  `src/ts/server/resourceInvalidation.test.ts` jointly protect the revision-to-projection contract.
- `src/ts/startupReadiness.test.ts`, reader workspace/intent tests, and the startup
...
- `resourceState.svelte.test.ts`, the [chat hydration suites](#chat-hydration-coverage-ownership), and
...
| --- | --- |
| Startup and writer ownership | `src/ts/bootstrap.test.ts`; `src/ts/bootstrap.readiness.dom.test.ts`; `src/ts/bootstrap.writerLifecycle.dom.test.ts`; `src/ts/bootstrap.writerSwitching.dom.test.ts`; `src/ts/bootstrap.errors.dom.test.ts`; `src/ts/startupReadiness.test.ts`; `src/ts/readerRouteIntent.test.ts`; `src/ts/readerProjectionLifecycle.test.ts`; `src/lib/Workspace.svelte.test.ts`; `src/ts/server/activeWriterSession.test.ts`; `bootstrap.test.ts` |
...
| Bootstrap response and event reconciliation | `src/ts/bootstrap.resourceEvents.dom.test.ts`; `src/ts/bootstrap.acknowledgements.settings.dom.test.ts`; `src/ts/bootstrap.acknowledgements.presets.dom.test.ts`; `src/ts/bootstrap.acknowledgements.collections.dom.test.ts`; `src/ts/bootstrap.acknowledgements.extensions.dom.test.ts`; `src/ts/bootstrap.acknowledgements.charactersAndChats.dom.test.ts` |
| Command and event transport | `src/ts/server/commands.test.ts`; `src/ts/server/commands.queue.test.ts`; `src/ts/server/commands.durableReplay.test.ts`; `src/ts/server/commands.translation.test.ts`; `events.test.ts`; `settingsDraftAcknowledgement.test.ts` |
| Domain command adapters | `src/ts/server/commands.settings.test.ts`; `src/ts/server/commands.presets.test.ts`; `src/ts/server/commands.modelProfiles.test.ts`; `src/ts/server/commands.agentPresets.test.ts`; `src/ts/server/commands.prompts.test.ts`; `src/ts/server/commands.personas.test.ts`; `src/ts/server/commands.translatorPresets.test.ts`; `src/ts/server/commands.loadouts.test.ts`; `src/ts/server/commands.characters.test.ts`; `src/ts/server/commands.chats.test.ts`; `src/ts/server/commands.messages.test.ts`; `src/ts/server/commands.chatGenerationSettings.test.ts`; `src/ts/server/commands.lorebooks.test.ts`; `src/ts/server/commands.definitions.test.ts`; `src/ts/server/commands.modules.test.ts`; `src/ts/server/commands.plugins.test.ts`; `src/ts/server/commands.bardWiki.test.ts` |
| Resource state and reads | `src/ts/server/characterShellHydration.test.ts`; `chatMessageHydration.reactivity.svelte.test.ts`; `chatMessageHydration.test.ts`; `chatMessageHydration.reader.dom.test.ts`; `chatMessageHydration.freshness.dom.test.ts`; `chatMessageHydration.bulk.dom.test.ts`; `chatMessageHydration.completion.dom.test.ts`; `characterLorebookHydration.dom.test.ts`; `hydrationReads.svelte-node.test.ts`; `promptTemplateHydration.test.ts`; `resourceCache.test.ts`; `resourceCacheDelivery.svelte-node.test.ts`; `resourceCache.workCosts.svelte-node.test.ts`; `resourceReads.svelte-node.test.ts`; `resourceState.svelte.test.ts`; `shellProtocol.test.ts`; `shellHydration.svelte-node.test.ts`; `resourceManifest.test.ts`; `routeResourceLoader.test.ts` |
