...
| Startup capabilities and reader lifecycle | `src/ts/startupReadiness.test.ts`, `readerRouteIntent.test.ts`, `readerProjectionLifecycle.test.ts`, `src/lib/Workspace.svelte.test.ts`, reader cases in `src/ts/bootstrap.test.ts`, and App route DOM coverage: monotonic milestones, narrow capability gates, denial/promotion, writer loss, local-only route intent, auth/replacement clearing, and accessible status/focus. | Allows managed Reader navigation while gating writer route effects, mutation and generation; failed promotion cannot replay completed writer work. |
