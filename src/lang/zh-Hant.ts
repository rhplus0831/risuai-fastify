export const languageChineseTraditional = {
  connectedReaders: {
    chatOccupancy: {
      unsupported: '無法使用僅聊天存取',
      unsupportedHelp: '此伺服器不支援僅聊天互動。你仍可繼續閱讀。',
      disabled: '僅聊天存取已關閉',
      disabledHelp: '暫時無法取得新的聊天存取權。你仍可繼續閱讀。',
      identityUnavailable: '此分頁無法使用僅聊天存取',
      identityUnavailableHelp: '請使用原分頁或重新載入此頁。你仍可在此閱讀。',
      available: '可在此裝置上使用',
      availableHelp: '請明確取得此聊天的使用權後再傳送或重新生成。僅瀏覽不會佔用聊天。',
      foreignOwned: '正在另一部裝置上使用',
      ...
      normalizationRequiredHelp: '失去一般寫入權限後，此裝置保留了多個聊天。互動前請保留目前聊天。',
      normalizationElsewhere: '請先選擇一個已保留的聊天',
      ...
      sendAccepted: '訊息已接受。',
      sendQueued: '此訊息仍保留在此裝置上；目前尚無法確認伺服器是否已接受。',
      sendFailed: '訊息未被接受。你的草稿仍保留在這裡。',
      rerollAccepted: '重新生成已接受。',
      rerollRetained: '重新生成要求仍保留在此裝置上；目前尚無法確認伺服器是否已接受。',
      rerollFailed: '重新生成未被接受。現有回覆未變更。',
      stopRequested: '已要求停止。正在等待伺服器協調回覆。',
      stopFailed: '無法確認停止。生成可能仍在進行。',
      actionUnavailable: '目前無法執行此操作。請重新整理後再試。',
      actionFailed: '無法變更聊天存取權。請重新整理後再試。',
      recoveryBlocked: '必須先等待已接受的工作完成或將其停止，才能釋放或切換此聊天。',
    },
  },
  generationRecovery: {
  ...
  help: {
    ...
    additionalAssets:
      '在對話中顯示的額外資源。\n\n - 使用 `{{raw::<資源名稱>}}` 作為路徑。\n - 使用 `{{image::<資源名稱>}}` 作為圖片。\n - 使用 `{{video::<資源名稱>}}` 作為影片。\n - 使用 `{{audio::<資源名稱>}}` 作為音訊。\n    - 建議放置在背景 HTML 中。',
    superMemory:
      'SupaMemory 通過向 AI 提供總結資料來增強角色的記憶能力。\n\nSupaMemory 是一項文字總結功能。不建議使用輔助模型，除非該模型無審查、最大上下文超過 2000 Token，且具有良好的總結能力。\n\nSupaMemory 提示詞決定了模型如何撰寫總結。留空將使用預設提示詞，建議保持留空。\n\n完成所有設定後，您可以在角色的設定中啟用此功能。',
    replaceGlobalNote: '此欄位不為空時，將替換當前的全域備註為此內容。',
    backgroundHTML:
      '將 Markdown/HTML 注入到對話視窗的背景中。\n\n您也可以使用額外資源。例如，您可以使用 {{audio::<資源名稱}} 作為背景音樂。\n\n此外，您還可以與額外資源搭配使用以下格式：\n - {{bg::<資源名稱>}}：將資源設為背景。',
    additionalText:
      '只有當 AI 認為有必要時，才會將額外描述加入至角色描述中。您可以在此處放置較長的文本。使用雙換行進行內容分隔。',
    charjs:
      '這是一段會與角色一同運行的 JavaScript。詳情請查看：https://github.com/kwaroran/risuai/blob/main/src/etc/example-char.js\n**出於安全原因，目前不建議使用。這些程式碼不會被包含在匯出中。**',
    romanizer:
      'Romanizer 是一個將非羅馬字母轉換為羅馬字母的外掛，用於減少發送資料時的 Token。這可能會導致輸出結果與原始模型不同。如果已在對話中使用羅馬字母，不建議啟用。',
    inlayImages: '啟用後，圖片可嵌入對話中，並且支援此功能的 AI 將能夠看到它們。',
    metrica:
      'Metric Systemizer 是一個外掛，會在請求階段將公制單位轉換為英制單位，而在輸出時轉回公制，讓使用者在介面上顯示公制單位，但效能上使用英制單位。如果在對話中已經使用英制單位，不建議啟用。',
    lorePlus:
      'LoreBook+ 是一項實驗性功能。使用向量資料庫（VectorDB），而不僅是字串匹配。目的在於提供更好的建立角色體驗和更好的匹配效能。',
    ...
    additionalParams:
      '此選項允許將附加的參數加入至請求主體（Request body）中。若要排除某些參數，可以將值設為 `{{none}}`。若要加入標頭（Request header）而非主體，可以在鍵前加上 `header::`，如 `header::Authorization`。若要將值作為 JSON，可以在值前加上 `json::`，如 `json::{"key":"value"}`。其他情況下，系統將自動判定值的類型。',
    antiClaudeOverload:
    ...
    claudeCachingExperimental:
      'Claude 快取是實驗性功能，可減少模型成本。但若在不使用重新生成回應的情況下啟用，則可能增加成本。實驗性功能可能不穩定，且未來可能會有所變動。',
    urllora:
      '可使用模型文件的直接下載鏈接。通過類似 https://sites.google.com/site/gdocs2direct/ 的網站，從 Google Drive 等平台生成直接連接。或者使用 Civitai URL，複製 AIR（格式如 `urn:air:flux1:lora:civitai:180891@776656` 或 `civitai:180891@776656`），並貼上。',
    ...
    summarizationPrompt:
      '用於總結的提示詞。留空將使用預設提示詞。您也可以使用包含 {{slot}} 的 ChatML 格式來處理對話資料。',
    translatorPrompt:
      '用於翻譯的提示詞。留空將使用預設提示詞。您還可以使用帶有 {{slot}} 的 ChatML 格式表示目標語言：用 {{slot::content}} 表示內容，用 {{slot::tnote}} 表示翻譯備註。',
    ...
    experimentalChatCompressionDesc:
      '壓縮未使用的對話資料並儲存在單獨的檔案中。這將大幅減少對話資料的大小，並顯著提升效能，但這是實驗性功能，可能會不穩定，並導致備份功能等出現問題。',
    promptInfoInsideChatDesc:
      '啟用後，將在對話中繼資料中儲存預設資訊。儲存的資料包括預設名稱、啟用的切換和提示詞文字。這可能會略微增加處理時間和儲存空間的使用。',
    ...
    reSummarizationPrompt:
      '批量編輯、將多個選定的總結合併為一個時使用的提示詞。如果留空將使用預設提示詞。總結輸出會以雙換行碼 (\\n\\n) 為基準分割成區塊，用於相似性搜尋。',
  ...
  setup: {
    ...
    claudeDesc:
      'Claude 是由 Anthropic 開發的 AI 模型，是 OpenAI 的強大競爭對手。若您希望使用非英語語言，它在各方面的表現往往優於 GPT。',
  ...
  triggerDesc: {
    ...
    v2ModifyLorebookByIndexDesc:
      '修改索引為 {{index}} 的 Lorebook，名稱為 {{name}}，關鍵字為 {{key}}，內容為 {{content}}',
  ...
  reroll: '重新生成',
  chatList: '對話列表',
  chatListExportAll: '匯出所有對話',
  chatListImport: '匯入對話',
  chatListEdit: '編輯對話列表',
  chatListCreateFolder: '建立對話資料夾',
  ...
  doNotWarnExternalServers: '不再警告外部伺服器',
  altGreet: '備選問候語',
  scripts: '指令碼',
  settings: '設定',
  selective: '選擇性',
  ...
  organizeCharacters: '整理角色',
  characterActionsFor: (name: string) => `整理 ${name}`,
  folderActionsFor: (name: string) => `${name} 的資料夾操作`,
  moveToFolder: '移動到資料夾',
  moveOutOfFolder: '移出資料夾',
  createFolderWith: '與其他角色建立資料夾…',
  chooseDestinationFolder: '選擇目標資料夾',
  chooseFolderPartner: '選擇要放入新資料夾的角色',
  newCharacterFolderName: '新資料夾',
  chooseFolderColor: '選擇資料夾顏色',
  resetFolderImage: '重設為預設圖片',
  selectFolderImageFile: '選擇圖片檔案',
  folderColorRed: '紅色',
  folderColorGreen: '綠色',
  folderColorBlue: '藍色',
  folderColorYellow: '黃色',
  folderColorIndigo: '靛藍色',
  ...
  donatorPatreonDesc:
    '為保護隱私，預設不會顯示在名單中。若想顯示您的暱稱，請前往 Risuai 的 Patreon 頁面並點擊連結按鈕。',
  ...
  hypaV3Modal: {
    ...
    searchAction: '搜尋摘要',
    previousSearchResultAction: '上一個搜尋結果',
    nextSearchResultAction: '下一個搜尋結果',
    importantFilterAction: '僅顯示重要摘要',
    bulkEditAction: '批次編輯摘要',
    categoryManagerAction: '管理分類',
    settingsAction: '開啟 HypaV3 設定',
    moreActionsAction: '更多操作',
    selectedFilterAction: '僅顯示所選摘要',
    resetDataAction: '重設 HypaV3 資料',
    closeAction: '關閉 HypaV3',
    selectSummaryAction: '選取總結 #{0}',
    deselectSummaryAction: '取消選取總結 #{0}',
    summaryCategoryLabel: '總結 #{0} 的分類',
    toggleSummaryTranslationAction: '顯示或隱藏總結翻譯',
    toggleSummaryImportantAction: '切換總結的重要標記',
    rerollSummaryAction: '重新生成總結',
    deleteSummaryAction: '刪除總結',
    deleteFollowingSummariesAction: '刪除後續總結',
    toggleRerolledSummaryTranslationAction: '顯示或隱藏重新生成總結的翻譯',
    cancelRerollAction: '取消重新生成總結',
    applyRerollAction: '套用重新生成的總結',
    toggleConnectedMessageTranslationAction: '顯示或隱藏關聯訊息翻譯',
    bulkCategoryLabel: '所選總結的分類',
    toggleBulkImportantAction: '切換所選總結的重要標記',
    bulkSelectionInputLabel: '要選取的總結編號',
    toggleResummaryTranslationAction: '顯示或隱藏重新總結翻譯',
    retryResummaryAction: '重試重新總結',
    applyResummaryAction: '套用重新總結結果',
    cancelResummaryAction: '取消重新總結',
    addCategoryAction: '新增分類',
    saveCategoryAction: '儲存分類',
    cancelCategoryEditAction: '取消編輯分類',
    editCategoryAction: '編輯分類',
    deleteCategoryAction: '刪除分類',
    tagNameLabel: '標籤名稱',
    saveTagAction: '儲存標籤',
    cancelTagEditAction: '取消編輯標籤',
    editTagAction: '編輯標籤',
    deleteTagAction: '刪除標籤',
    refreshMemoryJobsAction: '重新整理記憶體工作',
  ...
  settingsNavInputHooks: '輸入鉤子',
  inputHooks: '輸入鉤子',
  inputHookTypeDraft: '草稿',
  ...
  inputHookSelectDraft: '選擇草稿鉤子',
  inputHookDraftDialogTitle: '選擇草稿鉤子',
  inputHookNone: '無',
  inputHookBtw: 'BTW',
  inputHookBtwDialogTitle: '執行 BTW 鉤子',
  inputHookBtwResult: 'BTW 結果',
  inputHookBtwDismiss: '關閉 BTW 結果',
  inputHookSendDraft: '傳送草稿',
