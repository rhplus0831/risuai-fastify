...
proof recorded in findings. All Smoke Phase 3 scenario/control reviews are complete below;
its focused evidence and phase gate remain separate.
...
| `server/fastify/browser-smoke/connectedGenerationHarness.ts`, `connectedReaderRolloutHarness.ts`                                                   | Do local provider/response controls retain real ownership, persistence, replay and native draft provenance?    | Retained/strengthened; accepted Reader Phases 4–5 |
...
reviews the named pilots; final path/oracle dispositions are completed in
the Phase 2, Reader and Phase 3 records below. Per-spec contexts and control roles below apply to every
...
| S12 | `bardWikiLifecycle.spec.ts:39`                 | BardWiki settings, manual document, confirmation status, and lifecycle tools are visible end to end                   | Retained; Phase 3 complete review               |
...
| S22 | `chatHistoryScroll.spec.ts:55`                 | 300-message history stays readable with rapid reversals and pauses among tall messages                                | Strengthened; BSE-005 renewed controls verified |
...
| S25 | `chatStartupRendering.spec.ts:113`             | direct chat startup releases the newest rows before older display work and preserves their scroll anchor              | Retained; 2b review                             |
...
| S28 | `fastifyBrowserSmoke.spec.ts:128`              | Fastify-served browser loads bootstrap, subscribes to events, and refreshes after a command                           | Reclassified; browser/API companion             |
| S29 | `fastifyBrowserSmoke.spec.ts:345`              | authored settings survive local backup restore and a full reload                                                      | Retained; Phase 3 complete review               |
| S30 | `fastifyBrowserSmoke.spec.ts:420`              | authored character identity fields survive command acceptance and a full reload                                       | Retained; Phase 3 complete review               |
| S31 | `fastifyBrowserSmoke.spec.ts:487`              | translator preset bindings persist independently across chats                                                         | Retained; Phase 3 complete review               |
...
| S33 | `fastifyBrowserSmoke.spec.ts:639`              | core chat controls and blocking alerts remain accessible across responsive viewports                                  | Retained; Phase 3 complete review               |
...
| S35 | `fastifyBrowserSmoke.spec.ts:825`              | mobile in-flow composer opens from a button above the stable keyboard viewport                                        | Retained; Phase 3 complete review               |
| S36 | `fastifyBrowserSmoke.spec.ts:1157`             | prompt presets and model profiles reorder from an immediate mobile touch drag                                         | Retained; Phase 3 complete review               |
| S37 | `fastifyBrowserSmoke.spec.ts:1201`             | global lorebook page owner hydrates once, selects by stable id, and survives reload                                   | Retained; Phase 3 complete review               |
| S38 | `lazyFirstOpen.spec.ts:281`                    | smoke manifest accounts for the 60 registered lazy boundaries                                                         | Reclassified; 60-entry manifest consistency     |
| S39 | `lazyFirstOpen.spec.ts:306`                    | every Settings and Playground route opens its real first-use chunk                                                    | Retained; Phase 3 complete review               |
| S40 | `lazyFirstOpen.spec.ts:340`                    | grid, route handlers, Sidebar panels, and chat dialogs open only on first use                                         | Retained; Phase 3 complete review               |
| S41 | `lazyFirstOpen.spec.ts:416`                    | a delayed emitted stylesheet keeps the previous route mounted until the new route is ready                            | Retained; Phase 3 complete review               |
| S42 | `lazyFirstOpen.spec.ts:444`                    | a delayed modal chunk preserves focus through loading, CSS, and close                                                 | Retained; Phase 3 complete review               |
| S43 | `lazyFirstOpen.spec.ts:486`                    | preset and persona lazy dialogs stay within the viewport after first-open loading                                     | Retained; Phase 3 complete review               |
...
| S45 | `lazyFirstOpen.spec.ts:557`                    | a stale emitted stylesheet shows local recovery and reloads the current route                                         | Retained; Phase 3 complete review               |
...
| S47 | `selectedLocaleRuntime.spec.ts:23`             | a delayed locale cannot overwrite a newer selection and is reused on the next switch                                  | Retained; Phase 3 complete review               |
| S48 | `selectedLocaleRuntime.spec.ts:73`             | a failed locale chunk leaves the current UI usable and a later selection retries it                                   | Retained; Phase 3 complete review               |
| S49 | `selectedLocaleRuntime.spec.ts:108`            | cold selected-locale failure retries before exposing its first composer                                               | Retained; Phase 3 complete review               |
| S50 | `selectedLocaleStartup.spec.ts:15`             | selected locale is usable on cold startup and refresh                                                                 | Retained; Phase 3 complete review               |
| S51 | `startupCachePopulationMatrix.spec.ts:78`      | startup matrix keeps cold and warm small/large populations separate                                                   | Reclassified/strengthened; BSE-008 verified     |
| S52 | `startupDirectLinks.spec.ts:31`                | Fast-bootstrap direct-link matrix › batch 1/4 hydrates 11 empty-cache routes                                          | Reclassified; declared route hydration          |
| S53 | `startupDirectLinks.spec.ts:31`                | Fast-bootstrap direct-link matrix › batch 2/4 hydrates 11 empty-cache routes                                          | Reclassified; declared route hydration          |
| S54 | `startupDirectLinks.spec.ts:31`                | Fast-bootstrap direct-link matrix › batch 3/4 hydrates 11 empty-cache routes                                          | Reclassified; declared route hydration          |
| S55 | `startupDirectLinks.spec.ts:31`                | Fast-bootstrap direct-link matrix › batch 4/4 hydrates 11 empty-cache routes                                          | Reclassified; declared route hydration          |
| S56 | `startupRecoveryIntegrationMatrix.spec.ts:41`  | startup rollout matrix proves flag-off and flag-on boundaries on small and large fixtures                             | Retained; Phase 3 complete review               |
| S57 | `startupRecoveryIntegrationMatrix.spec.ts:61`  | legacy and null shell values normalize during built-browser startup                                                   | Reclassified; startup read normalization        |
...
| S61 | `startupRecoveryIntegrationMatrix.spec.ts:427` | background runtimes cannot delay or fail shell, mutation, and chat readiness                                          | Retained; Phase 3 complete review               |
| S62 | `startupRecoveryIntegrationMatrix.spec.ts:503` | inlay runtime stays route-local when slow or failed and recovers through Retry                                        | Retained; Phase 3 complete review               |
...
| S65 | `transcriptResidency.spec.ts:331`              | transcript residency screenshot is temporary full materialization                                                     | Retained; Phase 3 complete review               |
| S66 | `transcriptResidency.spec.ts:397`              | transcript residency preserves editing, selection and copy desktop                                                    | Retained; Phase 3 complete review               |
| S67 | `transcriptResidency.spec.ts:397`              | transcript residency preserves editing, selection and copy mobile                                                     | Retained; Phase 3 complete review               |
| S68 | `transcriptResidency.spec.ts:555`              | transcript residency restores ordinary rows after screenshot failure                                                  | Retained; Phase 3 complete review               |
| S69 | `transcriptResidency.spec.ts:555`              | transcript residency restores ordinary rows after screenshot cancellation                                             | Retained; Phase 3 complete review               |
| S70 | `transcriptResidency.spec.ts:610`              | transcript residency bounds eight editors through page reset and keyboard gap navigation                              | Retained; Phase 3 complete review               |
| S71 | `transcriptResidency.spec.ts:793`              | transcript residency cancels a pending jump when its route is hidden and reopened                                     | Retained; Phase 3 complete review               |
| S72 | `transcriptResidency.spec.ts:906`              | transcript legacy paging rollback traverses 180 mounted rows without spacers                                          | Retained; Phase 3 complete review               |
...
| S81 | `connectedWriterSwitching.spec.ts:725`         | Use this device switches A to B to A in place while preserving reader routes and the originating draft                | Strengthened; BSE-009 target race verified      |
| S82 | `connectedWriterSwitching.spec.ts:890`         | an accepted server generation keeps its job and durable reply when Use this device transfers the writer               | Strengthened; Reader Phase 3 verified           |
| S83 | `connectedWriterSwitching.spec.ts:1100`        | an empty server without Web Locks initializes only after the explicit setup action with a fresh writer identity       | Strengthened; Reader Phase 3 verified           |
| S84 | `connectedReaderGeneration.spec.ts:28`         | a connected Reader sees one live partial and the exact persisted reply without control or effect requests             | Strengthened; Reader Phase 4 verified           |
| S85 | `connectedReaderGeneration.spec.ts:62`         | Reader chat switching and close/reopen detach viewers while the same provider job keeps running                       | Strengthened; Reader Phase 4 verified           |
| S86 | `connectedReaderGeneration.spec.ts:122`        | writer transfers during streaming and stopping preserve one cancelled partial without completion effects              | Strengthened; Reader Phase 4 verified           |
| S87 | `connectedReaderGeneration.spec.ts:201`        | writer transfer while a real finalization journal is queued commits one result and settles one effect ledger          | Strengthened; Reader Phase 4 verified           |
| S88 | `connectedReaderGeneration.spec.ts:363`        | an accepted IGP append is already receipted when its writer loses ownership before the PATCH response                 | Strengthened; Reader Phase 4 verified           |
| S89 | `connectedReaderRollout.spec.ts:102`           | default connected Reader reconnects after an actual server restart without page reload or write takeover              | Strengthened; Reader Phase 5 fault verified     |
| S90 | `connectedReaderRollout.spec.ts:223`           | conservative fallback reload replays one UI-saved command and preserves the newer unsent composer draft               | Strengthened; Reader Phase 5 fault verified     |
| S91 | `visibleStateRecovery.spec.ts:229`             | connected-default import recovery preserves the character sidebar after explicit same-owner writer recovery           | Strengthened; Reader Phase 5 fault verified     |
| S92 | `selectedLocaleRuntime.spec.ts:151`            | new writer navigation reaches Settings while the initial character route handler is delayed                           | Strengthened; BSE-006 verified                  |
...
The shared harness starts the real Fastify app and authenticates through the
smoke password flow. Default handcrafted RisuSave setup uses the real auth-only
import into initialized, unowned disposable SQLite; it does not acquire a writer
before browser startup. Explicit writer-import, migration and empty-server modes
retain their separate SQL preconditions. The harness serves `dist` over a random
loopback port and closes/removes the owned data afterward.
...
memory worker in the shared harness. Embedded harness exclusions retain the per-owner
reviews below. These tests do not establish normal password prompts, worker timing, or
...
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `getDatabaseSnapshot`, `getLifecycleSnapshot`, `getAppliedServerResourceRevision`, `getStartupSnapshot`, `getStartupCoordinatorSnapshot`, `getClientSessionSnapshot`, `getCurrentRoute`, `getRouteResourceLoadState`, `getRerollCandidates` | Observation of actual client projections; requires a DOM or authoritative API oracle for claims of visible/durable truth.                                     |
| `isLoaded`, `waitForLoaded`, `waitForStartupMilestone`                                                                                                                                                                                      | Readiness observation; `isLoaded` means background-ready; it does not imply route-handler completion or a user action.                                        |
...
The concrete caller map below captures all hook member references in specs and local helpers at
...
| `chatStartupRendering.spec.ts`             | `waitForStartupMilestone`: 51, 97, 160                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `connectedGenerationHarness.ts`            | `activeWriterHeaders`: 555; `getClientSessionSnapshot`: 446; `getDatabaseSnapshot`: 578; `getStartupCoordinatorSnapshot`: 418, 491, 509; `waitForStartupMilestone`: 540                                                                                                                                                                                                                                                                                                  |
| `connectedReaderBrowsing.spec.ts`          | `activeWriterHeaders`: 184; `getAppliedServerResourceRevision`: 204; `getClientSessionSnapshot`: 494; `getLifecycleSnapshot`: 362, 493; `getStartupCoordinatorSnapshot`: 168, 249; `waitForStartupMilestone`: 246, 335                                                                                                                                                                                                                                                   |
| `connectedReaderGeneration.spec.ts`        | Through `connectedGenerationHarness.ts`; no direct member call.                                                                                                                                                                                                                                                                                                                                                                                                          |
| `connectedReaderRollout.spec.ts`           | `getLifecycleSnapshot`: 98, 321                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `connectedReaderRolloutHarness.ts`         | `getClientSessionSnapshot`: 188; `getLifecycleSnapshot`: 196; `getStartupCoordinatorSnapshot`: 137, 157; `waitForStartupMilestone`: 134                                                                                                                                                                                                                                                                                                                                  |
| `connectedWriterSwitching.spec.ts`         | `activeWriterHeaders`: 547; `getAppliedServerResourceRevision`: 635, 687; `getClientSessionSnapshot`: 476, 684; `getCurrentRoute`: 513, 685; `getDatabaseSnapshot`: 912; `getGenerationReadinessDiagnostic`: 515, 682; `getRouteResourceLoadState`: 686; `getStartupChatReadinessEvaluations`: 477, 516, 683; `getStartupCoordinatorSnapshot`: 315, 335, 396, 514, 681, 1196, 1212; `waitForStartupMilestone`: 360, 1207                                                 |
| `debugEchoLayoutStability.spec.ts`         | `activeWriterHeaders`: 193; `getDatabaseSnapshot`: 230, 241; `isLoaded`: 187                                                                                                                                                                                                                                                                                                                                                                                             |
| `displayPaintCache.spec.ts`                | `getDatabaseSnapshot`: 156, 168; `waitForStartupMilestone`: 80, 163                                                                                                                                                                                                                                                                                                                                                                                                      |
| `fastifyBrowserSmoke.spec.ts`              | `activeWriterHeaders`: 240, 617; `getDatabaseSnapshot`: 226, 311, 408, 459, 483, 519, 520, 555; `patchRuntimeSettings`: 230, 629; `selectCharacter`: 652, 742, 870; `showAlert`: 698; `waitForLoaded`: 196, 651, 1233                                                                                                                                                                                                                                                    |
| `lazyFirstOpen.spec.ts`                    | `navigateTo`: 425, 538, 580, 651; `setQuickSettingsOpen`: 384, 388; `showAlert`: 459; `waitForLoaded`: 662                                                                                                                                                                                                                                                                                                                                                               |
| `realmProgressConfirmation.spec.ts`        | `waitForStartupMilestone`: 313                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| `selectedLocaleRuntime.spec.ts`            | `getClientSessionSnapshot`: 204; `getDatabaseSnapshot`: 49, 59; `getRouteResourceLoadState`: 206; `getStartupCoordinatorSnapshot`: 205; `navigateTo`: 17, 62, 99, 227; `waitForStartupMilestone`: 16, 141, 212                                                                                                                                                                                                                                                           |
| `selectedLocaleStartup.spec.ts`            | `getStartupSnapshot`: 46; `waitForStartupMilestone`: 43                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `startupCachePopulationMatrix.spec.ts`     | `getPendingResourceCacheWriteCount`: 157; `getStartupCoordinatorSnapshot`: 201; `getStartupSnapshot`: 209; `waitForStartupMilestone`: 197                                                                                                                                                                                                                                                                                                                                |
...
| `startupRecoveryIntegrationMatrix.spec.ts` | `activeWriterHeaders`: 258; `getAppliedServerResourceRevision`: 143, 185, 256, 309; `getDatabaseSnapshot`: 96, 182, 285, 310, 411, 470; `getLifecycleSnapshot`: 165, 179; `getRouteResourceLoadState`: 543, 552, 559; `getStartupCoordinatorSnapshot`: 317, 390, 462, 483, 537, 562, 609, 640; `getStartupSnapshot`: 474, 639; `navigateTo`: 534; `patchRuntimeSettings`: 162, 376, 398, 404, 466; `waitForStartupMilestone`: 93, 140, 176, 253, 369, 382, 480, 515, 628 |
...
| `visibleStateRecovery.spec.ts`             | `activeWriterHeaders`: 682; `getAppliedServerResourceRevision`: 672; `getClientSessionSnapshot`: 154, 300, 350, 405, 457; `getDatabaseSnapshot`: 99, 139, 217, 353; `getLifecycleSnapshot`: 423; `getStartupCoordinatorSnapshot`: 417; `patchRuntimeSettings`: 173, 318; `selectCharacter`: 550; `waitForLoaded`: 546                                                                                                                                                    |
...
[reader status](../../ui-and-user-input/connected-read-only-clients/status.md#phase-2-implementation-2026-09-07)
...
acceptance in [reader status](../../ui-and-user-input/connected-read-only-clients/status.md).
...
The corrected `60ac61bde` campaign now passes all seventeen positive executions,
qualifies three expected negatives across the two named faults, and renews the
actual compiled-FALSE fallback with exact normal emission restoration. The
[completed repair proof](findings.md#renewed-combined-controls-and-fallback)
retains the original failed baseline, prepared-pause and completion-ordering
...
## Phase 3 Complete Scenario and Control Review

This final review supersedes the historical pending/partial dispositions above.
The parent reconciled five read-only review assignments: three completed and two
timeout scopes were fully reviewed manually. Recommendations were checked against
source. In particular, S31's `openChat` uses `page.goto`, so returning to its first
chat is a full document navigation; it was not missing all reconstruction proof.
No required product gap is deferred by these dispositions. The unchanged lower-risk
journeys retain accurate limits instead of acquiring unrelated fault matrices.

| Cases       | Actual entry, producer and independent outcome                                                                                                                                                                                        | Retained scope and failure boundary                                                                                                                                                                              |
| ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| S12         | Real BardWiki Settings UI creates a manual document with accepted POST and visible title, confirms it with accepted POST/status, and reloads the created document. Fixture seeds configuration and two messages.                      | Retained authoring/confirmation/reconstruction; lifecycle tool text is visible, but worker apply/rebuild execution is disabled. Memory/projection and authoring suites remain separate companions.               |
| S28         | Real built bootstrap/events plus a durable hook settings command; direct authenticated browser API calls exercise echo, assets, memory, export/import, with accepted responses, refresh and cache/lineage observations.               | Reclassified as browser/API integration. Seeded data and a tiny uploaded image do not prove UI generation, image rendering or import controls. S01, S29 and S78/S79 own those named UI paths.                    |
| S29         | UI toggles authored settings, waits for accepted PATCH, creates a real local-backup download, changes the value, imports those downloaded bytes via file chooser, then reloads the visible restored checkbox.                         | Retained backup round trip for the named authored boolean; not all backup fields or native download-permission behavior.                                                                                         |
| S30         | Actual character editor authors name/description/greeting, receives accepted PATCH, then reloads and displays all three exact fields and stable character ID.                                                                         | Retained authoring/reconstruction; original fixture fields are preconditions and do not supply the authored result.                                                                                              |
| S31         | Actual per-chat selectors save distinct preset IDs via two accepted chat PATCHes. Full navigation back to the first chat reconstructs its binding; both projected bindings are compared.                                              | Retained independent binding contract. Translator presets are seeded; no provider translation result or separate second-chat reload assertion is claimed.                                                        |
| S33         | Responsive 1280×800/390×844 controls, composer geometry, no nested buttons, Tab/OK focus behavior and four PNG baselines. Alert is supplied by the presentation hook.                                                                 | Retained accessibility/presentation; the alert-store control does not exercise an operation's confirmation admission. S78/S79 own that boundary.                                                                 |
| S35         | Actual floating composer button, menu hide/reopen and Go to Bottom preserve draft/focus/history anchors while controlled `visualViewport` events exercise invalid height, valid height and cached keyboard geometry.                  | Retained viewport/composer contract with exact bounds. The viewport is simulated; no physical IME or continuous touch-scroll claim.                                                                              |
| S36         | Pixel emulation sends real CDP touch start/eight moves/end through both list controls; accepted reorder POSTs and exact visible preset/model order are checked.                                                                       | Retained immediate touch drag and command acceptance. No full reload, physical device or unrelated long-press contract.                                                                                          |
| S37         | Real global-lorebook page performs one focused GET, actual stable-ID selection sends its command, and full reload preserves the second selected book in the UI.                                                                       | Retained owner/hydration/selection/reconstruction. Seeded lore entries are not an authored import test.                                                                                                          |
| S38         | Static comparison of the curated 60 registered lazy sources, manifest dynamic-entry flags and Playground import reachability.                                                                                                         | Reclassified as registered-manifest consistency. The corrected title no longer claims every production lazy boundary or visible first open.                                                                      |
| S39         | Production router sweeps all 23 Settings cases, Playground menu and 13 child cases; independent asset requests and visible ready surfaces establish first use and shared-chunk reuse.                                                 | Retained route/chunk loading. Router hook bypasses menu entry and page-domain semantics; generated route aliases do not inflate the registered count.                                                            |
| S40         | Grid/character use the router; character sidebar, developer tools, chat list and module-chat controls use clicks; Quick Settings uses direct UI-state setup. Each named surface requires its first-use asset and readiness.           | Retained mixed entry/loader scope. No universal overlay coverage or page-error-free claim is inferred beyond these assertions.                                                                                   |
| S41         | Real emitted Settings CSS is held across production navigation; previous-route sentinel and shell remain, new surface stays absent, then loaded CSS/gradient and ready Settings appear.                                               | Retained stylesheet readiness and previous-route preservation; not every action on the previous screen.                                                                                                          |
| S42         | Actual Alert chunk/CSS requests are held; pending modal, busy state, inert opener, focus trap, visible text, OK and focus return are checked.                                                                                         | Retained lazy modal/focus. Alert-store dispatch and synthetic opener are explicit controls, not operation confirmation proof.                                                                                    |
| S43         | Actual model/persona picker controls open visible bounded dialogs. Model chunk delivery is independently held and observed.                                                                                                           | Retained picker viewport/loading; persona lacks a separate asset-request oracle and is not claimed as complete first-use asset proof.                                                                            |
| S44/S45     | Conservative offline Grid first open exposes local error/Retry and recovers after reconnection. Separate actual Settings CSS 404 exposes local recovery; real Reload requests current CSS and restores the route.                     | Retained distinct Retry/reload paths. Browser network controls supply the faults; no physical network loss or actual deployment-skew reproduction.                                                               |
| S47/S48/S49 | Native language select races held Korean against English, reuses the pack on reselection; separate actual 503 retries via selection; cold preselected Korean failure suppresses the first composer until acknowledged Retry succeeds. | Retained visible locale ordering/reuse/retry. No failed-selection durable rollback or all-language claim. Initial route repair and its independent held-handler companion are S92/BSE-006.                       |
| S50         | en/ko × three repetitions × cold/warm startup; a fresh real imported harness per pair, first localized composer, readiness and emitted script budget/receipts.                                                                        | Retained all twelve samples. Imported selected locale is the precondition; this does not author the selection or cover every pack.                                                                               |
| S51         | Small/generated-large fixture × cold new context/cache clear versus same-context reload; cache counters, payload/timing/readiness and zero early write requests.                                                                      | Reclassified as cache/telemetry measurement. Aggregate bytes/hits are not independent hydrated-content correctness.                                                                                              |
| S52–S55     | Four real isolated batches cover 44 direct `page.goto` URLs after browser/application cache clears; current route, resource readiness, required/forbidden API paths and errors are observed.                                          | Reclassified as manifest-declared hydration/routing. Expected resources share the production manifest, so a joint omission is not independently detected; no page-domain DOM or universal completeness claim.    |
| S56         | Four small/large × disabled/enabled imported fixtures hold the first real writer bootstrap. Observer DOM/mutation-generation gates, actual readiness order, one completion telemetry and zero early writes are checked.               | Retained explicit rollout/capability matrix. Its overrides are not normal-default proof; S89/S90/S91 separately establish that.                                                                                  |
| S57         | After real harness import/start, SQL supplies legacy/null/invalid shell values. Built startup reads the normalized client projection and records no page errors.                                                                      | Reclassified as read normalization during startup. `normalizeSettingsForRead` clones nonrepairing reads; neither persisted repair nor repair before bootstrap is claimed. Title corrected; assertions unchanged. |
| S61         | Real optional Display GET is held or returns 503 while shell/mutation/chat capabilities and a real hook-driven settings command remain usable; release/failure state is independently observed.                                       | Retained optional-resource isolation, not all background runtimes or a settings-control UI action.                                                                                                               |
| S62         | Production Inlay navigation with held/503 GET displays local loading/error while shell/mutation remain usable; release or actual Retry reaches ready.                                                                                 | Retained route-local recovery, not asset import/rendering or a navigation button.                                                                                                                                |
| S65         | Actual screenshot menu temporarily materializes all logical rows, produces PNG download and restores bounded ordinary residency.                                                                                                      | Retained materialization/download lifecycle; PNG pixel/content completeness is not asserted.                                                                                                                     |
| S66/S67     | Actual edit/save/copy/branch controls preserve draft/focus/selected text and exact visible edit, then folded/unfolded branch residency and anchor bounds.                                                                             | Retained desktop/mobile interaction/residency. DOM Range/scroll controls and replaced plaintext clipboard are explicit; no native HTML clipboard permissions or branch reload durability.                        |
| S68/S69     | Real screenshot action encounters controlled canvas-encoding failure or hook-driven route cancellation; ordinary rows return within bounds and no download appears.                                                                   | Retained failure/cancellation restoration; route control bypasses a navigation button and does not assert a separate cancellation receipt.                                                                       |
| S70         | Eight actual editors, keyboard gap entry and hook-driven page reset retain drafts and bounded residency; accepted PATCHes and authoritative messages GET verify saved edits.                                                          | Retained editor cap/keyboard/reset/real save contract. Settings reset is setup rather than the Settings UI.                                                                                                      |
| S71         | Real bookmark/Go to Chat starts a jump into a controlled delayed-image gate; production router hides/reopens the route, latest content remains visible/bounded after delayed completion.                                              | Retained jump cancellation/lifetime; source fixture PATCH is setup, not independent authoring proof.                                                                                                             |
| S72         | Explicit legacy localStorage mode plus direct geometry and synthetic scroll/wheel traverse all 180 mounted rows with no spacers.                                                                                                      | Retained conservative residency rollback; it does not replace S21/S22's continuous real input.                                                                                                                   |
| S80–S92     | Reader browsing, switching, generation, restart/fallback, connected lineage recovery and delayed initial route retain their separately recorded real DOM/API/SQL controls and qualified faults.                                       | Strengthened/retained at the completed Reader handoff, including S22/BSE-005 and S92/BSE-006 renewed controls. Source-limited earlier results are not universal coverage.                                        |

S01–S11, S13–S27, S32, S34, S46, S58–S60, S63/S64, S73–S79 retain the
complete Phase 2/reader maintenance reviews above. Stage 3 rechecks affected
critical journeys at the final implementation: normal send/completed reload,
continuous reversal/readable pause, both Realm decisions, conservative recovery
and connected recovery. The two title corrections are static/scope corrections;
assertions and product source are unchanged. No new production fault is claimed
for a title edit. Changed material contracts already have their named baseline,
negative and restored evidence in BSE-005/BSE-006 and Reader Phase 5 findings.

### Final support and conditional ownership

All twelve local TypeScript support files, embedded fixtures/helpers, four PNGs,
the shared hook/type, smoke-only production switches and execution/artifact owners
have retained or strengthened dispositions. The Phase 1 review still owns auth,
fixture import, manifest-derived route expectations, result completeness and
worker isolation; later Reader records own the new generation/rollout helpers.
The final caller map includes direct accesses and local aliases in specs and
support helpers. `getClientSessionSnapshot` is a read-only dispatch/authority
observation, never an assignment of role or a server-authority substitute.
`connectedReaderGeneration.spec.ts` obtains these controls through its shared
harness. S92's `background-ready` is explicitly distinct from route readiness.

Default fixture import is now auth-only/unowned and each fresh context pair owns
its durable harness where identity requires it. Explicit legacy takeover/setup
fixtures retain their declared modes. Browser-owned contexts close before their
servers, preventing idle HTTP pools from delaying cleanup after product success.
Memory workers and asset GC remain disabled; local provider controls retain real
coordinator/SSE/SQLite behavior. The 100ms smoke finalization refresh remains a
timing difference. Conditional transcript costs/profiles/legacy exclusions,
...
### Final Follow-Up Control and Source Reconciliation

Fresh discovery after `985da9bc8` again matches all 92 identities/22 specs, twelve
local TypeScript support files and four PNGs. The source-anchor and twenty-four
hook-owner maps above are current, including S81's native scheduling helper and
the three passive diagnostic getters. S51 and S81's follow-ups are now verified
by [BSE-008](findings.md#bse-008-warm-cache-requires-completed-optional-writes)
and [BSE-009](findings.md#bse-009-writer-startup-must-follow-the-current-hydration-target).
These results supersede their pending follow-up entries above.

| Control / caller                                                 | Role and independent limit                                                                                                                                                                                                                                                                                                                                                                                                   |
| ---------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| S51 `getPendingResourceCacheWriteCount`                          | Observation of actual admitted optional writes, used as setup before warm reload after cold metrics are captured. It neither flushes nor supplies cache contents. Original hit/miss and payload assertions remain independent consequences.                                                                                                                                                                                  |
| S81 `getStartupChatReadinessEvaluations`                         | Observation of real concurrent startup evaluations, current session generation, semantic target and awaited phase; cloned metadata cannot mutate or complete the owner. It distinguishes startup's B await from an eager B request.                                                                                                                                                                                          |
| Writer `getGenerationReadinessDiagnostic` and terminal snapshots | Observations of native coordinator/session/route/failure state, supplementing actual UI/HTTP/SQL oracles. They do not supply ready state, select a chat or acquire ownership.                                                                                                                                                                                                                                                |
| S81 `holdNativeGetResponses`                                     | External response-delivery scheduling at actual A-detail/B-message GETs, with unchanged real Fastify response bodies. Promotion still uses both actual UI confirmation actions. The same pending B evaluation, native visible A route, accepted A SQL selection and subsequent B HTTP 200 prove the tested order. Successful helper fulfillment is distinguished from browser receipt; the superseded old A read may cancel. |

All original S81 draft/recovery, stale-client denial, A/B selection and ownership,
message durability and no-reload assertions remain. The accepted-generation and
empty-server siblings remain unchanged and pass in both full three-case writer
cohorts. S01/S51/S89–S92 also pass both repetitions before/after the qualified
writer fault, and two current actual compiled-FALSE fallback controls preserve
